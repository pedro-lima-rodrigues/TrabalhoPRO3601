...
rec_transf MyTransf(municipios.Layout Le) := TRANSFORM
  ...
  SELF.preco_combustivel := (decimal)STD.Str.FindReplace(Le.preco_medio_revenda, ',','.'); //PROBLEMA AQUI
  SELF := Le;
END;
...
rec_transf2 MyTransf2(inicial.Layout Le) := TRANSFORM
  ...
  SELF.hub_city := if(Le.hub_city<>'PORTO ALEGRE' OR Le.hub_city<>'CURITIBA' OR Le.hub_city<>'RIO DE JANEIRO','SAO PAULO',Le.hub_city);
  SELF := Le;
END;
...
Join1:= JOIN(inicial_transf,municipios_transf,
                LEFT.data_atual>RIGHT.data_inicial1 AND LEFT.data_atual<RIGHT.data_final1 AND LEFT.hub_city=RIGHT.municipio, 
                MyJoin(LEFT, RIGHT),
